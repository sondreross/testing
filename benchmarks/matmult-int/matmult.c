...
#define RANDOM_VALUE (RandomInteger_int ())
#define ZERO 0
#define MOD_SIZE 8095
typedef long matrix [UPPERLIMIT][UPPERLIMIT];

static int
values_match_int (long v1, long v2)
{
  return (v1 == v2);
}
...
static int Seed_int;
static matrix ArrayA_int, ArrayB_int, ResultArray_int;

static void Multiply_int(matrix A, matrix B, matrix Res);
static void InitSeed_int(void);
static void Test_int(matrix A, matrix B, matrix Res);
static int RandomInteger_int(void);

int matmult_int()
{
   for (int i = 0; i < BENCHMARK_REPEAT; i++) {
      Test_int(ArrayA_int, ArrayB_int, ResultArray_int);
   }
...
static int RandomInteger_int(void)
{
   Seed_int = ((Seed_int * 133) + 81) % MOD_SIZE;
   return (Seed_int);
}
...
void initialise_benchmark_matmult_int() {
   ...
   for (OuterIndex = 0; OuterIndex < UPPERLIMIT; OuterIndex++)
      for (InnerIndex = 0; InnerIndex < UPPERLIMIT; InnerIndex++)
         ArrayA_int[OuterIndex][InnerIndex] = RANDOM_VALUE;
   for (OuterIndex = 0; OuterIndex < UPPERLIMIT; OuterIndex++)
      for (InnerIndex = 0; InnerIndex < UPPERLIMIT; InnerIndex++)
         ArrayB_int[OuterIndex][InnerIndex] = RANDOM_VALUE;
}
...
int verify_benchmark_matmult_int(int unused)
{
  ...
  for (i=0; i<UPPERLIMIT; i++)
    for (j=0; j<UPPERLIMIT; j++)
      if (ResultArray_int[i][j] != exp[i][j])
        return 0;
  return 1;
}
...
char* get_benchmark_name_matmult_int(void) {
    return "matmult-int";
}
